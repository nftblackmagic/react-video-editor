...
export function buildGroupEDUsMessages(paragraph: string): ModelMessage[] {
	return [
		{
			role: "user",
			content: [
				{
					type: "text",
					text: `You are an expert in Rhetorical Structure Theory (RST). Your task is to analyze the following passage by splitting it into Elementary Discourse Units (EDUs) and find unprocessed words.
...
2. There might be incomplete words at end of the user provided paragraph. 
**If you find it's an incomplete sentence or phrase, you should find the unprocessed words and return them.**
**Unprocessed words can only exist at the end of the paragraph.**
**If you find the unprocessed words or phrase, you should return them as unprocessed_words. They cannot be regarded as EDUs.**
...
4. How to apply (concise rules):
Segment EDUs first. Split by clauses/minimal units. Break "Although X, Y" into two EDUs (X and Y).
Prefer a single tag. If truly necessary, allow a primary/secondary pair (e.g., EV[primary] + EX[secondary]).

5. Decision order (quick flow):
Calling for action or stating consequences? → IM
Stating the thesis the audience must accept? → CL
Supporting a claim? → data/research = EV; concrete case = EX
Acknowledging limits/opponents? → CS
Actively countering a view? → RB
None of the above and mostly setting/definitions? → BG
Incomplete sentence or phrase at end of the paragraph? → unprocessed words

6. Boundary notes & small tweaks:
EX vs EV. Two workable options:
A. Keep both (recommended): research/stats = EV; concrete case = EX.
B. Merge under EV with subtypes, e.g., EV:stat / EV:example.
...
10. Optional helpers (project-dependent):
TR = Transition (e.g., "In sum", "Next"), NA = Non-propositional (greetings, IDs, station tags).

11. Labeled paragraph example (EDU → tag):
"Urban noise has worsened in recent years." → BG
"We should improve nighttime construction management." → CL
"City environmental reports show complaints rose 30%." → EV
"For example, one neighborhood reported three incidents in a month." → EX
...
"But multi-site studies find little impact on timelines." → RB
"Therefore, cease high-noise work after 10:00 p.m." → IM
"It should be noted that" at end of the paragraph → unprocessed words


12. Output Requirements:
- Preserve all original text exactly as written, do not add any new content. Do not delete any content.
- All punctuation should be the same as the original text. For example, "，" should be preserved as "，". Do not change the punctuation.
- Ensure EDU boundaries align with natural linguistic units
- Each EDU should be self-contained and meaningful
- If there are unprocessed words, they should be returned in the unprocessed_words field

Example analysis:
Input: "Climate change poses significant risks.这个问题就得到了解决啊。We must act now.值得一"
