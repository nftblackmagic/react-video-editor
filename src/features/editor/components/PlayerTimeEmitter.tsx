...
const PlayerTimeEmitter = () => {
	...
	useEffect(() => {
		...
		const playSubscription = subject
			.pipe(filter(({ key }) => key === PLAYER_PLAY))
			.subscribe(() => {
				startEmitting();
			});

		// Listen for pause event
		const pauseSubscription = subject
			.pipe(filter(({ key }) => key === PLAYER_PAUSE))
			.subscribe(() => {
				stopEmitting();
			});
	...
	useEffect(() => {
		const seekSubscription = subject
			.pipe(
