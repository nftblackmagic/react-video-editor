...
class Audio extends Trimmable {
  ...
  public _render(ctx: CanvasRenderingContext2D) {
    ...
    const displayFromInUnits = timeMsToUnits(
      this.display?.from ?? 0,
      this.tScale
    );
    ...
    ctx.drawImage(
      ...
      0,
      0,
      (this.offscreenCanvas as OffscreenCanvas).width,
      this.height,
      visibleStart,
      0,
      (this.offscreenCanvas as OffscreenCanvas).width,
  ...
  public renderToOffscreen(force?: boolean) {
    ...
    (this.offscreenCanvas as OffscreenCanvas).width = MAX_CANVAS_WIDTH;
    (this.offscreenCanvas as OffscreenCanvas).height = this.height;
    ...
    for (let i = startBarIndex; i < endBarIndex && i < bars.length; i++) {
      ...
      if (bar) {
        ...
        if (x >= 0 && x < (this.offscreenCanvas as OffscreenCanvas).width) {
