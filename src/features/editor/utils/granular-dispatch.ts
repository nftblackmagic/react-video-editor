...
export function dispatchLoadTracks(tracks: any[]) {
	...
	try {
		...
		if (!validation.valid) {
			console.error(
				"Invalid payload for dispatchLoadTracks:",
				validation.errors,
			);
			return false;
		}
...
export function dispatchLoadTrackItems(
	...
	try {
		...
		if (!validation.valid) {
			console.error(
				"Invalid payload for dispatchLoadTrackItems:",
				validation.errors,
			);
			return false;
		}
...
export function dispatchLoadTransitions(
	...
	try {
		...
		const payload = {
			...
			trackItemsMap:
				existingData?.trackItemsMap || existingData?.trackItems || {}, // Handle both property names
			trackItemIds:
				existingData?.trackItemIds ||
				Object.keys(
					existingData?.trackItemsMap || existingData?.trackItems || {},
				),
		...
		if (!validation.valid) {
			console.error(
				"Invalid payload for dispatchLoadTransitions:",
				validation.errors,
			);
			return false;
		}
...
export function dispatchLoadCompositions(
	...
	try {
		...
		const payload = {
			...
			transitionsMap:
				existingData?.transitionsMap || existingData?.transitions || {},
			transitionIds:
				existingData?.transitionIds ||
				Object.keys(
					existingData?.transitionsMap || existingData?.transitions || {},
				),
			compositions,
		...
		if (!validation.valid) {
			console.error(
				"Invalid payload for dispatchLoadCompositions:",
				validation.errors,
			);
			return false;
		}
