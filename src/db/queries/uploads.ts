...
export async function getUserUploads(
  ...
  let query = db.select().from(uploads).where(eq(uploads.userId, userId));

  if (options?.projectId) {
    query = query.where(
      and(eq(uploads.userId, userId), eq(uploads.projectId, options.projectId))
    );
  }
  ...
  if (options?.fileType) {
    query = query.where(
      and(
        ...
        like(uploads.fileType, `${options.fileType}%`)
      )
    );
  }
  ...
  if (options?.status) {
    query = query.where(
      and(eq(uploads.userId, userId), eq(uploads.status, options.status))
    );
  }

  query = query.orderBy(desc(uploads.createdAt));
...
export async function getUserMediaStats(userId: string) {
  ...
  const stats = {
    ...
    byType: {
      videos: userUploads.filter((u) => u.fileType?.startsWith("video/")).length,
      images: userUploads.filter((u) => u.fileType?.startsWith("image/")).length,
