...
export async function getUserTranscriptionStats(userId: string) {
	...
	const stats = {
		...
		completed: userTranscriptions.filter((t) => t.status === "completed")
			.length,
		processing: userTranscriptions.filter((t) => t.status === "processing")
			.length,
		failed: userTranscriptions.filter((t) => t.status === "failed").length,
		totalWords: userTranscriptions.reduce(
			(sum, t) => sum + (t.wordCount || 0),
			0,
		),
		totalDuration: userTranscriptions.reduce(
